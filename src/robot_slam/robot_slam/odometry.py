...
class OdomNode(Node):
    ...
    def odom_update(self):
        ...
        # Use single timestamp for both odom message and TF to ensure perfect sync
        current_time = now.to_msg()
        
        odom_msg = Odometry()
        odom_msg.header.stamp = current_time
        odom_msg.header.frame_id = 'odom'
        odom_msg.child_frame_id = 'base_link'  # Match SLAM config!

        odom_msg.pose.pose.position.x = self.x
        odom_msg.pose.pose.position.y = self.y
        odom_msg.pose.pose.position.z = 0.0
        
        # Complete quaternion for pure yaw rotation
        ...
        # Broadcast TF with SAME timestamp as odom message
        transform = TransformStamped()
        transform.header.stamp = current_time
        transform.header.frame_id = 'odom'
        transform.child_frame_id = 'base_link'  # Match SLAM config!
