...
# class GoalPoseToNav(Node):
#     def __init__(self):
#         super().__init__('goal_pose_to_nav')
#         self.sub = self.create_subscription(PoseStamped, '/goal_pose', self.send_goal, 10)
#         self.client = ActionClient(self, NavigateToPose, '/navigate_to_pose')
#         self.get_logger().info('Use RViz "2D Goal Pose" tool to send navigation goals')
...
#         self.get_logger().info(f"Sending goal: ({msg.pose.position.x:.2f}, {msg.pose.position.y:.2f})")
#         self.client.send_goal_async(goal_msg)
